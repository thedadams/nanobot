...
type CallbackHandler interface {
	...
	NewStateWithRedirect(context.Context, *oauth2.Config, string, string) (string, error)
}
...
func (s *callbackHandler) NewStateWithRedirect(_ context.Context, conf *oauth2.Config, verifier, redirectURL string) (string, error) {
	state := strings.ToLower(rand.Text())
	s.lock.Lock()
	defer s.lock.Unlock()
